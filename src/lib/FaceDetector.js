...
export default class FaceDetector extends Component {
  ...
  //   var imgURL
  //   this.canvas.getContext('2d', { alpha: false })
  //   console.log("clicked:", this.canvas.toDataURL())
  //   imgURL = this.canvas.toDataURL()
  //   console.log("img url : ", imgURL)

  //   this.setState({ GetImage: imgURL})
  ...
  relativeFaceLocation = (faceData) => {
    ...
    if (faceData && faceData.x) {
      ...
      // console.log("y:", faceData.face.height)
  ...
  updateCanvas = () => {
    ...
    if (this.state.facesData[0]) {
      this.state.facesData.map(face => {
        this.ctx.beginPath();
        // this.ctx.fillRect(face.x, face.y, face.size,  face.strength)
        // this.ctx.arc(face.x, face.y, face.size / 2, 0, 2 * Math.PI, false);
        // this.ctx.rect(face.x, face.y, face.size,  face.strength)
